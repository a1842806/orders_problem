...
pub async fn top_customers(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    let rows = sqlx::query("SELECT user_id, COUNT(*) as order_count FROM orders GROUP BY user_id ORDER BY order_count DESC LIMIT 10")
        .fetch_all(pool)
        .await?;
...
pub async fn total_customers(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    let row = sqlx::query("SELECT COUNT(DISTINCT user_id) FROM orders")
        .fetch_one(pool)
        .await?;
...
pub async fn customer_with_most_closed_orders(pool: &sqlx::PgPool) -> Result<(), sqlx::Error> {
    let row = sqlx::query("SELECT user_id, COUNT(*) as order_count FROM orders WHERE status = 'CLOSED' GROUP BY user_id ORDER BY order_count DESC LIMIT 1")
        .fetch_one(pool)
        .await?;
